    ...
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> ExternalLoginConfirmation(ExternalLoginConfirmationViewModel model, string returnUrl)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (info.ExternalIdentity.AuthenticationType == "Google")
                {
                    firstName = info.ExternalIdentity.Claims.First(c => c.Type.Contains("givenname")).Value ??
                     string.Empty;
                }
                var user = new ApplicationUser
                {
                    UserName = model.Email,
                    Email = model.Email,
                    FirstName = (firstName.Substring(0, Math.Min(firstName.Length, 15)))?? model.FirstName,
                    LastName = model.LastName
        ...
        [AllowAnonymous]
        [ValidateAntiForgeryToken]
        public async Task<ActionResult> SignUpDj(RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                var user = new ApplicationUser
        ...
        [AllowAnonymous]
        [ValidateAntiForgeryToken]
        public async Task<ActionResult> SignUpHt(RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                var user = new ApplicationUser
